        ...
        loop {
            ...
            if !updater_registered {
                match create_updater_source(&installed_tools_service, &directory_manager).await {
                    ...
                    None => {
                        debug!(
                            "Updater not installed yet, will retry in {}s",
                            SOURCE_DISCOVERY_INTERVAL_SECS
                        );
                    }
                }
...
async fn create_updater_source(
    ...
    // the path the updater binary writes to; a SERVICE-type tool has no
    // executable_path, so presence in installed_tools is the discovery signal.
