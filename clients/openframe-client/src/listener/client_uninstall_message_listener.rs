    ...
    async fn handle_message(
        ...
        if !cfg!(any(target_os = "macos", target_os = "windows")) {
            ...
            message.ack().await.map_err(|e| {
                anyhow::anyhow!("Failed to ack unsupported-platform message: {}", e)
            })?;
            return Ok(());
        }
            ...
            .await
        {
            ...
            Err(e) => warn!(
                "Failed to delete client uninstall consumer {} (continuing with uninstall): {:#}",
                durable_name, e
            ),
        }
        ...
        loop {
            ...
            for attempt in 1..=CONSUMER_RETRY_ATTEMPTS_PER_CYCLE {
                    ...
                    .await
                {
                    Ok(consumer) => {
                        info!(
                            "Client uninstall consumer created for stream: {}",
                            Self::STREAM_NAME
                        );
                        return consumer;
                    }
                    Err(e) => {
                            ...
                            || error_msg.contains("10013")
                        {
                            ...
                            {
                                info!(
                                    "Retrieved existing client uninstall consumer for stream: {}",
                                    Self::STREAM_NAME
                                );
