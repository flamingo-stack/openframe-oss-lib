...
impl ToolInstallationService {
    ...
    async fn install_inner(
        ...
        let mesh_db_backup = ReinstallBackupGuard(
            if reinstall && tool_agent_id == "meshcentral-agent" {
        ...
        if let Some(backup) = mesh_db_backup.0.as_ref() {
            ...
            } = &installation
            {
                ...
                if let Err(e) = crate::platform::system_service::stop_service(svc, false).await {
                    warn!(
                        "TEMP: failed to stop {} before agent.db restore: {:#}",
                        svc, e
                    );
                }
                tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
                match fs::copy(backup, &db_path).await {
                    ...
                    Err(e) => warn!(
                        "TEMP: failed to restore mesh agent.db from {}: {:#}; NodeID will rotate",
                        backup.display(),
                        e
                    ),
                }
