...
fn script_message_without_script_id_still_parses() {
    let m =
        ScriptMessage::from_payload(r#"{"executionId":"e","code":"x","shell":"BASH"}"#).unwrap();
...
fn missing_privilege_defaults_to_admin() {
    let m =
        CommandMessage::from_payload(r#"{"executionId":"e","code":"x","shell":"CMD"}"#).unwrap();
    assert!(matches!(
        m.to_requests()[0].privilege,
...
fn result_serializes_snake_case() {
    ...
    assert!(
        v.get("schedule_id").is_none(),
        "None schedule_id must be omitted"
    );
}
...
fn schedule_results_reuse_the_script_execution_subject() {
    ...
    assert_eq!(
        ScriptScheduleExecutionMessage::RESULT_KIND,
        "script-execution"
    );
