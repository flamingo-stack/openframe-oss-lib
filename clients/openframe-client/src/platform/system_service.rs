...
use crate::config::service_stop::SERVICE_START_RETRY_DELAYS_SECS;
...
/// Delay before the attempt after `attempt` (1-based); None once the backoff schedule is exhausted.
#[cfg_attr(not(target_os = "windows"), allow(dead_code))]
fn service_start_retry_delay(attempt: usize) -> Option<Duration> {
    SERVICE_START_RETRY_DELAYS_SECS
        .get(attempt.checked_sub(1)?)
        .map(|secs| Duration::from_secs(*secs))
}

/// Codes no retry can fix: 2/3 image path missing, 1058 disabled, 1060 does not exist, 1072 marked for delete.
#[cfg_attr(not(target_os = "windows"), allow(dead_code))]
fn is_permanent_service_start_code(code: i32) -> bool {
    matches!(code, 2 | 3 | 1058 | 1060 | 1072)
}

/// Outcome of one failed start attempt; decides whether the loop backs off or bails.
#[cfg(target_os = "windows")]
enum ServiceStartFailure {
...
impl ServiceStartFailure {
    fn from_scm(stage: &str, e: &windows_service::Error) -> Self {
        let message = format!("{}: {}", stage, scm_error_detail(e));
        match e {
            windows_service::Error::Winapi(io)
                if io
                    .raw_os_error()
                    .is_some_and(is_permanent_service_start_code) =>
            {
    ...
    fn message(&self) -> &str {
        match self {
            Self::Transient(m) | Self::Permanent(m) | Self::ScmUnresponsive(m) => m,
        }
    }
}
...
pub async fn start_service(service_name: &str) -> Result<()> {
    ...
    let total_attempts = SERVICE_START_RETRY_DELAYS_SECS.len() + 1;
    let mut last_err = String::new();
    for attempt in 1..=total_attempts {
        let name = service_name.to_string();
        let start_result = scm_call_timed(
            service_name,
            "start call",
            SERVICE_START_CALL_TIMEOUT_SECS,
        ...
        match service_start_retry_delay(attempt) {
            Some(delay) => {
                info!(
                    "Retrying start of service {} in {}s",
                    service_name,
                    delay.as_secs()
                );
                sleep(delay).await;
    ...
    for _ in 1..=SERVICE_STOP_MAX_ATTEMPTS {
        ...
        match query_service_status_timed(service_name).await {
            ...
            Err(e) => {
                query_failures += 1;
                warn!(
                    "Status query for service {} failed while awaiting RUNNING: {e:#}",
                    service_name
                );
...
#[cfg(target_os = "windows")]
fn try_start_service_windows(service_name: &str) -> std::result::Result<(), ServiceStartFailure> {
    use winapi::shared::winerror::ERROR_SERVICE_ALREADY_RUNNING;
    use windows_service::service::ServiceAccess;
    use windows_service::service_manager::{ServiceManager, ServiceManagerAccess};

    let manager = ServiceManager::local_computer(None::<&str>, ServiceManagerAccess::CONNECT)
        .map_err(|e| ServiceStartFailure::from_scm("connect to SCM", &e))?;
    let service = manager
        .open_service(
            service_name,
            ServiceAccess::START | ServiceAccess::QUERY_STATUS,
        )
        .map_err(|e| ServiceStartFailure::from_scm("open service", &e))?;
    ...
    match service.start::<&std::ffi::OsStr>(&[]) {
        ...
        Err(e) => Err(ServiceStartFailure::from_scm("start call", &e)),
    }
}
...
pub async fn service_clear_for_install(service_name: &str) -> bool {
    #[cfg(target_os = "windows")]
    {
        match query_service_status_timed(service_name).await {
            ...
            Err(e) => {
                warn!(
                    "Status query for service {} failed during install check: {e:#}",
                    service_name
                );
                false
            }
...
async fn stop_service_windows(service_name: &str, allow_delete: bool) -> Result<()> {
    ...
    let stop_result = match scm_call_timed(
        service_name,
        "stop call",
        SERVICE_STOP_CALL_TIMEOUT_SECS,
        move || {
            let service =
    ...
    .await
    {
        Err(e) => {
            warn!(
                "service.stop() for {} did not complete ({e:#}); force-killing service process",
                service_name
            );
...
async fn force_stop_service_windows(service_name: &str, allow_delete: bool) -> Result<()> {
    for attempt in 1..=SERVICE_FORCE_KILL_MAX_ATTEMPTS {
        let status = match query_service_status_timed(service_name).await {
            ...
            Err(e) => {
                warn!(
                    "Status query for service {} failed during force-stop (attempt {}/{}): {e:#}",
            ...
            .and_then(|s| s.process_id)
        {
            ...
            None => {
                ...
                let name = service_name.to_string();
                let image_path = scm_call_timed(
                    service_name,
                    "config query",
                    SCM_QUERY_TIMEOUT_SECS,
                    move || service_image_exe_path_windows(&name),
                )
                .await
                .unwrap_or_else(|e| {
                    warn!(
                        "Config query for service {} failed during force-stop: {e:#}",
                        service_name
                    );
                    None
                });
    ...
    for attempt in 1..=SERVICE_STOP_MAX_ATTEMPTS {
        ...
        match query_service_status_timed(service_name).await {
            ...
            Err(e) => {
                query_failures += 1;
                warn!(
                    "Status query for service {} failed while awaiting stop: {e:#}",
                    service_name
                );
                if query_failures >= SCM_QUERY_MAX_CONSECUTIVE_FAILURES {
                    warn!(
                        "SCM unresponsive for service {}; escalating to force-stop",
                        service_name
                    );
                    return Ok(false);
                }
...
async fn delete_service_windows(service_name: &str) -> Result<()> {
    ...
    if service_missing_timed(service_name)
        .await
        .unwrap_or_else(|e| {
            warn!(
                "Existence query for service {} failed before delete: {e:#}",
                service_name
            );
            false
        })
    ...
    scm_call_timed(
        ...
        "delete call",
        SCM_QUERY_TIMEOUT_SECS,
        move || open_service_windows(&name, ServiceAccess::DELETE)?.delete(),
    )
    .await?
    ...
    for _ in 1..=SERVICE_STOP_MAX_ATTEMPTS {
        match service_missing_timed(service_name).await {
            ...
            Err(e) => {
                query_failures += 1;
                warn!(
                    "Existence query for service {} failed while confirming deletion: {e:#}",
                    service_name
                );
                if query_failures >= SCM_QUERY_MAX_CONSECUTIVE_FAILURES {
                    return Err(anyhow::anyhow!(
