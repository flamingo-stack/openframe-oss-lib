...
class NotificationChannelDispatcherTest {
    ...
    @Test
    @DisplayName("dispatch delivers to every user on every registered channel — settings were already applied to the audience upstream")
    ...
    @Test
    @DisplayName("Given no channel is registered (push module absent), when dispatch runs, then it is a quiet no-op")
