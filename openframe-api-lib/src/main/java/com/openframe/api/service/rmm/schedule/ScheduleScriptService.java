...
public class ScheduleScriptService {
    ...
    public CountedGenericQueryResult<ScriptScheduleResponse> list(ScriptScheduleFilterInput filter,
        ...
        List<ScheduleScript> items = hasMore ? page.subList(0, limit) : page;
    ...
    public ScriptScheduleResponse update(UpdateScriptScheduleInput input) {
        ...
        String tenantId = tenantIdProvider.getTenantId();
        ScheduleScript existing = loadVisibleOrThrow(tenantId, id);
    ...
    private static void validateOfflineBehavior(ScheduleScriptTrigger trigger, ScheduleTimeReference timeReference,
        ...
        if (reconnectWindowSeconds == null || reconnectWindowSeconds <= 0) {
            throw new BadRequestException(
                    "RETRY_ON_RECONNECT requires a positive reconnect window (reconnectWindowSeconds)");
        }
        if (repeatSeconds != null && reconnectWindowSeconds >= repeatSeconds) {
            throw new BadRequestException(
                    "reconnectWindowSeconds (" + reconnectWindowSeconds
                            + ") must be less than the schedule's repeat interval (" + repeatSeconds + " seconds)");
        }
    }
    ...
    private ScheduleScript loadOrThrow(String tenantId, String id) {
        return scheduleRepository.findByTenantIdAndId(tenantId, id)
                .orElseThrow(() -> new NotFoundException("Script schedule not found: " + id));
    }

    private ScheduleScript loadVisibleOrThrow(String tenantId, String id) {
        ScheduleScript schedule = loadOrThrow(tenantId, id);
