...
impl ToolInstallationService {
    ...
    pub async fn install(&self, tool_installation_message: ToolInstallationMessage) -> Result<()> {
        ...
        if file_path.exists() {
            info!(
                "Agent file for tool {} already exists at {}, skipping download",
                tool_agent_id,
                file_path.display()
            );
        } else {
            // Download main tool agent file
            let tool_agent_file_bytes = if let Some(ref download_configs) =
                tool_installation_message.download_configurations
            {
                // Use GithubDownloadService with download configurations
                info!("Using download configurations to download tool agent");
                let download_config =
                    GithubDownloadService::find_config_for_current_os(download_configs)
                        .with_context(|| {
                            format!(
                                "Failed to find download configuration for current OS for tool: {}",
                                tool_agent_id
                            )
                        })?;
                ...
                self.github_download_service
                    ...
                    .with_context(|| {
                        format!(
                            "Failed to download and extract tool agent for: {}",
                            tool_agent_id
                        )
                    })?
            } else {
                ...
                self.tool_agent_file_client
                    ...
                    .await
                    .with_context(|| {
                        format!("Failed to download tool agent file for: {}", tool_agent_id)
                    })?
            };
            ...
            File::create(&file_path)
                .await?
                .write_all(&tool_agent_file_bytes)
                .await?;
            ...
            info!(
                "Agent file for tool {} downloaded and saved to {}",
                tool_agent_id,
                file_path.display()
            );
        }
                    ...
                    AssetSource::ToolApi => {
                        ...
                        let resolved_path =
                            self.url_params_resolver.process(path).with_context(|| {
                                format!("Failed to resolve URL parameters for asset: {}", asset.id)
                            })?;
        ...
        let installed_tool = InstalledTool {
            ...
            session_type: tool_installation_message
                .session_type
                .clone()
                .unwrap_or(crate::models::SessionType::Service),
