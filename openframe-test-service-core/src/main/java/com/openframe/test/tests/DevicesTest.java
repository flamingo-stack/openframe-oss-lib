...
public class DevicesTest extends BaseTest {
    ...
    @Tag("read")
    @Test
    @DisplayName("Get mesh info")
    public void testMeshInfo() {
        Machine device = DeviceApi.getAnyDevice(onlineDevicesFilter(), offlineDevicesFilter());
        assertThat(device).as("No devices").isNotNull();
        device = DeviceApi.getDevice(device.getMachineId());
        String meshId = getMeshId(device);
        assertThat(meshId).withFailMessage("No MeshCentral agent on " + device.getHostname()).isNotBlank();
        MeshDevice mesh = DeviceApi.getMeshInfo(meshId);
        assertThat(mesh).as("No mesh info for meshId " + meshId).isNotNull();
