...
describe('sanitizeLandingUrl', () => {
  ...
  it('drops the fragment, where implicit-flow tokens live', () => {
    expect(sanitizeLandingUrl('https://x.com/cb#access_token=SECRET&token_type=bearer')).toBe('https://x.com/cb');
  });
  ...
  it.each([
    ...
    // A rejected URL is a different failure than a leaked secret — name it,
    // rather than running `not.toContain` against `undefined`.
    if (out === undefined) throw new Error(`sanitizeLandingUrl rejected "${url}" outright — expected a stripped URL`);
...
describe('captureFirstTouchAttribution', () => {
  ...
  it('uses sessionStorage, never localStorage', () => {
    ...
    // Vacuously true where the runtime does not expose localStorage (see the
    // note above); the sessionStorage assertion is the load-bearing half either
    // way. Written as one unconditional assertion so the test cannot silently
    // stop checking anything.
    expect(localStorageOrNull?.getItem(FIRST_TOUCH_ATTRIBUTION_KEY) ?? null).toBeNull();
  });
});
...
describe('withFirstTouchAttribution', () => {
  ...
  it('lets an explicit body value WIN over the stored one', () => {
    captureFirstTouchAttribution();
    expect(withFirstTouchAttribution({ utm_source: 'typed-by-form' }).utm_source).toBe('typed-by-form');
  });

  it('strips empty values so a blank never reaches the API', () => {
    expect(withFirstTouchAttribution({ email: 'a@b.com', phone: '', name: null })).toEqual({
      email: 'a@b.com',
    });
  });
