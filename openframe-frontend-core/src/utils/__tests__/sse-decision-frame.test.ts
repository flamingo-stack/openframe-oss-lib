...
describe('readLeadingDecisionFrame', () => {
  ...
  it('rejects when the first frame has the wrong kind', async () => {
    ...
    const wire = bytes(JSON.stringify(frame) + '\0');
    await expect(readLeadingDecisionFrame(streamingResponse([wire]))).rejects.toThrow(/expected decision_resolved/i);
  });

  it('rejects when the leading JSON is malformed', async () => {
    const wire = bytes('not-json{\0');
    await expect(readLeadingDecisionFrame(streamingResponse([wire]))).rejects.toThrow(/leading JSON parse failed/i);
  });
