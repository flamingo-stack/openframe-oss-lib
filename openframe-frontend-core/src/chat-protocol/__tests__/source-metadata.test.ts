...
import { isSourceMetadataCardRef, sourceMetadataEvent } from '../source-metadata';
...
describe('source metadata card provenance', () => {
  it('is recognized by a separate package entrypoint module instance', async () => {
    ...
    vi.resetModules();
    const { isSourceMetadataCardRef: isSourceMetadataCardRefFromSecondEntrypoint } = await import('../source-metadata');

    expect(isSourceMetadataCardRefFromSecondEntrypoint(ref)).toBe(true);
  });

  it('keeps source provenance after a serializing host boundary', () => {
    const event = sourceMetadataEvent({
      sources: [
        {
          index: 1,
    ...
    const restoredRef = structuredClone(ref);

    expect(isSourceMetadataCardRef(restoredRef)).toBe(true);
  });
  ...
  it('does not recognize an arbitrary enriched card ref', async () => {
    ...
    expect(
      isSourceMetadataCardRefFromSecondEntrypoint({
        type: 'markdown',
        id: 'unmatched-doc',
