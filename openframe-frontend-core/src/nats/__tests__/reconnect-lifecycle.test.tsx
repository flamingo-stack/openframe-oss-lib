...
class FakeWebSocket implements Pick<
  WebSocket,
  'url' | 'readyState' | 'binaryType' | 'bufferedAmount' | 'close' | 'send'
> {
  url: string;
...
function App({
  ...
  onBeforeReconnect,
  onProviderState,
  // Optional for callers that never mount the chat surface; defaulted here so
  // `ChatSurface`'s required `onState` gets a real function either way.
  onChatState = () => {},
  ...
  // Published through a callback prop, exactly like `onProviderState` /
  // `onChatState` — the spec drives this component's state through the handles
  // it hands out, rather than the component reaching into a parameter object
  // and writing its setters onto it.
  useEffect(() => {
    onControls({ setToken, setChatOpen });
  }, [onControls]);

  const getWsUrl = useCallback(() => (token ? currentUrl(token) : null), [token]);
...
function ChatSurface({
  ...
  // No url means no connection — a fact about this render, so it masks the
  // observed value instead of being written back into it from the effect.
  const [observedConnected, setConnected] = useState(false);
  const connected = Boolean(wsUrl) && observedConnected;
  const heldRef = useRef<string>('');
  const cbRef = useRef({ getWsUrl, onBeforeReconnect });
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // The shared client is the external system and `onStatusChange` below is
    // the subscription to it; this is that subscription's initial read, through
    // the same callback.
    const syncConnected = () => setConnected(conn.client.isConnected());
    syncConnected();
...
describe('shared NATS connection reconnect', () => {
  ...
  function boot(
    ...
    // Latest handles published by `App`. The facade below is what specs hold,
    // so `app.controls.setToken(...)` always reaches the CURRENT setter without
    // anything mutating a prop.
    let live: Controls = {
      setToken: () => {},
      setChatOpen: () => {},
    };
    const controls: Controls = {
      setToken: t => live.setToken(t),
      setChatOpen: v => live.setChatOpen(v),
    };
    render(
      <App
        onControls={c => {
  ...
  describe('token rotation', () => {
    it(
      'reconnects with the rotated token after the gateway closes at JWT expiry',
      async () => {
    ...
    it(
      'swaps cleanly when a REST 401 rotates the token while the socket is healthy',
      async () => {
    ...
    it(
      'recovers when the rotation lands on an already dead socket',
      async () => {
        const app = boot(noRefresh);
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
    ...
    it(
      'recovers when the rotation lands while a retry connect is in flight',
      async () => {
    ...
    it(
      'settles on the newest token when two rotations land back to back',
      async () => {
        const app = boot(noRefresh);
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
    ...
    it(
      'brings a chat surface back with the provider after a JWT-expiry close',
      async () => {
    ...
    it(
      'brings a chat surface across a rotation on a healthy socket',
      async () => {
  ...
  describe('reuse of a cached connection', () => {
    ...
    it(
      'dials again when the URL returns to a cached value inside the release grace',
      async () => {
        const app = boot(noRefresh);
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
    ...
    it(
      'dials again when the URL returns after the release grace has expired',
      async () => {
        const app = boot(noRefresh);
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
    ...
    it(
      'reconnects when a chat surface re-opens on the same URL with the socket dead',
      async () => {
        const app = boot(noRefresh, { withChat: true, chatOpenInitially: true });
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
        await waitFor(() => expect(app.chat.connected).toBe(true), { timeout: READY_TIMEOUT });
  ...
  describe('retry ownership across consumers', () => {
    ...
    it(
      'keeps retrying when a chat surface unmounts mid-backoff (chat owned the loop)',
      async () => {
        const app = boot(noRefresh, { withChat: true, chatOpenInitially: true });
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
        await waitFor(() => expect(app.chat.connected).toBe(true), { timeout: READY_TIMEOUT });
    ...
    it(
      'keeps retrying when a NON-owner chat surface unmounts mid-backoff',
      async () => {
        ...
        await waitFor(() => expect(getSharedConnectionFor(currentUrl('T1'))?.retryTimer ?? null).not.toBeNull(), {
    ...
    it(
      'lets a re-opened chat surface join the connection the survivor restored',
      async () => {
        const app = boot(noRefresh, { withChat: true });
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
        validTokens = new Set(['T1']);
    ...
    it(
      'recovers when a chat surface unmounts and the token then rotates',
      async () => {
        const app = boot(
          async () => {
    ...
    it(
      'does not pass the loop between lifecycles that have all moved to another URL',
      async () => {
        const url = currentUrl('T1');
        const movedOn = currentUrl('T2');
        let refreshes = 0;
        // Sync: the contract is `() => Promise<void> | void` and this counter
        // needs no async work of its own.
        const onBeforeReconnect = () => {
          refreshes += 1;
        };
    ...
    it(
      'recovers from a drop after a rotation moved every lifecycle',
      async () => {
        const app = boot(noRefresh, { withChat: true });
        await waitFor(() => expect(app.provider.ready).toBe(true), { timeout: READY_TIMEOUT });
        validTokens = new Set(['T1', 'T2']);
  ...
  describe('status mapping', () => {
    ...
    it(
      'reports staleConnection as disconnected, with no phantom reconnect',
      async () => {
        ...
        function StaleApp() {
          const getWsUrl = useCallback(() => currentUrl('T1'), []);
          return (
            <NatsProvider
              getWsUrl={getWsUrl}
