...
function Swatch({ name, className, darkLabel }: SwatchProps) {
  return (
    <div className="flex flex-col gap-1">
      <div className={`${className} flex h-16 w-full items-end justify-start rounded-md border border-ods-border p-2`}>
        <span className={`text-caption font-mono ${darkLabel ? 'text-black/70' : 'text-white/80'}`}>
          {name.replace(/^bg-|^text-|^border-/, '')}
        </span>
      </div>
      <span className="text-caption font-mono text-ods-text-secondary">{name}</span>
    </div>
  );
}
...
function TokenGrid() {
  return (
    <section className="space-y-6">
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Surfaces</h3>
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Text</h3>
        <div className="space-y-1 rounded-lg border border-ods-border bg-ods-card p-4">
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Accent &amp; status</h3>
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Borders</h3>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          <div className="text-caption flex h-16 items-center justify-center rounded-md border-2 border-ods-border bg-ods-card font-mono text-ods-text-secondary">
            border-ods-border
          </div>
          <div className="text-caption flex h-16 items-center justify-center rounded-md border-2 border-ods-border-hover bg-ods-card font-mono text-ods-text-secondary">
            border-ods-border-hover
          </div>
          <div className="text-caption flex h-16 items-center justify-center rounded-md border-2 border-ods-border-focus bg-ods-card font-mono text-ods-text-secondary">
...
function ComponentsShowcase() {
  return (
    <section className="space-y-6">
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Buttons</h3>
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Inputs</h3>
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Badges</h3>
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Cards</h3>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Card>
            <CardHeader>
              <CardTitle>Card title</CardTitle>
              <CardDescription>Card surface and text adapt to the active theme.</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-body-sm text-ods-text-secondary">
                All colors come from <code className="text-ods-accent">--ods-*</code> tokens — the same markup renders
      ...
      <div>
        <h3 className="mb-3 text-ods-text-primary text-h5">Alerts</h3>
        <div className="max-w-2xl space-y-3">
          <Alert>
            <AlertTitle>Informational</AlertTitle>
            <AlertDescription>Default alert surface — uses card background and primary text tokens.</AlertDescription>
          </Alert>
        </div>
      </div>
    </section>
...
export const Showcase: Story = {
  render: () => (
    <ThemeProvider>
      <div className="min-h-screen space-y-8 bg-ods-bg p-6 text-ods-text-primary transition-colors md:p-10">
        <header className="space-y-2">
          <h1 className="text-ods-text-primary text-h2">ODS theme switching</h1>
          <p className="text-body max-w-2xl text-ods-text-secondary">
            One <code className="text-ods-accent">data-theme</code> attribute on{' '}
            <code className="text-ods-accent">&lt;html&gt;</code> flips every{' '}
            <code className="text-ods-accent">--ods-*</code> primitive. Components below don&apos;t know — and
            don&apos;t care — which theme is active; they read tokens.
          </p>
        </header>
...
export const ToggleOnly: Story = {
  render: () => (
    <ThemeProvider>
      <div className="flex min-h-screen items-center justify-center bg-ods-bg p-10 text-ods-text-primary transition-colors">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle>Theme toggle</CardTitle>
            <CardDescription>Click the button below — the entire surface, text and border swap themes.</CardDescription>
          </CardHeader>
  ...
  render: () => (
    <div className="grid min-h-screen grid-cols-1 md:grid-cols-2">
      {(['light', 'dark'] as const).map(mode => (
        <div
          key={mode}
          className={`theme-${mode} space-y-4 border-r border-ods-border bg-ods-bg p-6 text-ods-text-primary`}
        >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Both halves render identical JSX — only the wrapping class differs.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
