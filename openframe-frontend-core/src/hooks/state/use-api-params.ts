...
export function useApiParams<TSchema extends ParamSchema>(
  ...
  const params = useMemo((): InferParamsFromSchema<TSchema> => {
    ...
    for (const [key, config] of Object.entries(schema)) {
      // Read from URL
      const rawValue = config.type === 'array' ? searchParams.getAll(key) : searchParams.get(key);
  ...
  const urlSearchParams = useMemo((): URLSearchParams => {
    ...
    for (const [key, _config] of Object.entries(schema)) {
  ...
  const setParam = useCallback(
    ...
    [schema, updateUrl, addParamToSearchParams, isEmptyValue],
  );
