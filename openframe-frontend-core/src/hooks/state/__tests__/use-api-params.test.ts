...
describe('useApiParams', () => {
  ...
  describe('setParam', () => {
    ...
    it('should warn when setting unknown parameter', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Unknown parameter: unknown'));
  ...
  describe('Debug mode', () => {
    ...
    it('should log URL updates when debug is enabled', () => {
      ...
      // `console.log` is declared `(...data: any[])`, so a call frame carries no
      // useful type. Widen it to `unknown[]` and narrow with a real guard rather
      // than reading `.includes` off an `any`.
      const urlUpdateCall = consoleSpy.mock.calls.find(
        (call: unknown[]) => typeof call[0] === 'string' && call[0].includes('[useApiParams] Updating URL:'),
      );
