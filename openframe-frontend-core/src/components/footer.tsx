...
function UniversalFooter({
  ...
  const defaultRenderLink = (link: FooterLink) => (
    <a
      href={link.href}
      className="font-body font-medium text-md md:text-md leading-[1.33] text-ods-text-primary hover:text-ods-accent-primary transition-colors"
    >
      {link.label}
    </a>
  );
  ...
  return (
    <footer
      className={`w-full flex flex-col justify-center items-center ${config.backgroundColor || 'bg-ods-bg-card'} px-6 py-10 relative gap-6 md:gap-6 min-h-[auto] md:min-h-[248px] z-50 border-t border-ods-border`}
    >
      <div className="w-full grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 items-start">
        ...
        <div className="flex flex-col gap-4 md:gap-6 items-start text-left col-span-2 md:col-span-1 lg:col-span-1">
          ...
          <div className="flex items-center gap-2">
            {config.logo && <Suspense fallback={<Skeleton className="w-8 h-8" />}>{config.logo as any}</Suspense>}
            {config.nameElement || (
              <span className="font-heading text-heading-5 font-bold text-ods-text-primary whitespace-nowrap">
                {config.name}
              </span>
            )}
          </div>
          ...
          {config.moveDescriptionToRight && config.keepBelowDescriptionLeft && config.belowDescriptionContent && (
            <Suspense fallback={<Skeleton className="h-8 w-full" />}>{config.belowDescriptionContent as any}</Suspense>
          )}
        </div>
        ...
        {config.sections.map((section, index) => (
          <div key={index} className="flex flex-col gap-3 items-start text-left col-span-1">
            <h3 className="text-h5 tracking-[-0.02em] text-ods-text-muted">{section.title}</h3>
        ...
        {config.customComponent && (
          <div className="flex flex-col col-span-2 md:col-span-1 lg:col-span-1 justify-center">
            <Suspense fallback={<Skeleton className="h-32 w-full" />}>{config.customComponent as any}</Suspense>
          </div>
        )}
        ...
        {(config.rightColumnContent || config.moveDescriptionToRight) && (
          <div className="flex flex-col col-span-2 md:col-span-1 lg:col-span-1 justify-start gap-4 md:gap-6">
            ...
            {config.rightColumnContent && (
              <Suspense fallback={<Skeleton className="h-32 w-full" />}>{config.rightColumnContent as any}</Suspense>
            )}
          </div>
        )}
      </div>
