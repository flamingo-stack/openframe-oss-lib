    ...
    >
      {showMedia ? (
        ...
        <div className="absolute inset-0" inert={isClone || undefined}>
          {/* Poster layer. Preferred: the bite's REAL thumbnail (generated by
              the admin bites editor's auto-backfill — the Vizard pipeline
              provides no cover). Fallback for bites without one: a
              metadata-only <video> paints the `#t=0.1` frame (media-fragment
              trick; works on iOS Safari where a fragmentless metadata load
              stays blank). Either way the box is never a black rectangle, and
              original + clone paint the IDENTICAL image so the marquee's wrap
              seam stays pixel-invisible. */}
          {bite.thumbnail_url ? (
            <img
