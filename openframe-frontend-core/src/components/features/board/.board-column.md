...
| `onToggleCollapse` | `(columnId: string) => void \| undefined` | Toggle handler — takes the id so one stable handler can serve every lane. Absent = the lane cannot collapse and renders no collapse arrow |
...
| `touchMode` | `boolean` | The touch-pager variant: registers NO DnD at all (no lane drop target, no `autoScrollColumn`, cards forced non-draggable) and words the empty state without "drag"; header, collapse and load-more are the same lane |
| `className` | `string` | Merged last onto the lane root, so the touch pager can override the fixed `w-[400px]` |
