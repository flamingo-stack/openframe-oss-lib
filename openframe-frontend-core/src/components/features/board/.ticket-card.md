...
|---|---|---|
| `TicketCardBody` | Component | Pure presentational inner content — title, device/org row, priority flag, assignees, tags, the `<ticketNumber> • <relative time>` line (either half alone when the other is missing; with `href` it is a link to the ticket, since it takes pointer events for its tooltip and would otherwise swallow the card's click), new message badge, and approval row |
...
| `TicketCardBodyProps` | Interface | Props for the shared body: `ticket`, `columnColor`, `renderAssignSlot`, `onApprove`, `onReject`, and optional `href` / `onLinkClick` for the number/time line |
