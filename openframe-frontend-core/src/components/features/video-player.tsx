...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  if (!mounted) {
    return (
      <div className={`video-player-container ${className}`}>
        <div className="video-wrapper relative w-full" style={useNativeAspectRatio ? {} : { paddingBottom: '56.25%' }}>
          <div
            className={
              useNativeAspectRatio
                ? 'loading-overlay bg-ods-card border border-ods-border rounded-md flex items-center justify-center min-h-[200px]'
                : 'loading-overlay absolute inset-0 bg-ods-card border border-ods-border rounded-md flex items-center justify-center'
            }
          >
        ...
        <div className="error-state bg-ods-card border border-ods-border rounded-md p-6 text-center">
          <div className="error-icon flex justify-center mb-4">
            <svg
              ...
              fill="currentColor"
              viewBox="0 0 24 24"
              className="text-ods-attention-red-error"
            >
  ...
  return (
    <div className={`video-player-container ${className}`}>
      {/* Video Title */}
      {title && showTitle && (
        <div className="video-title font-sans text-lg font-medium text-ods-text-primary mb-3">{title}</div>
      )}
      ...
      <div className="video-wrapper relative w-full" style={useNativeAspectRatio ? {} : { paddingBottom: '56.25%' }}>
        ...
        {isLoading && (
          <div
            className={
              useNativeAspectRatio
                ? 'loading-overlay bg-ods-card border border-ods-border rounded-md flex items-center justify-center z-10 min-h-[200px]'
                : 'loading-overlay absolute inset-0 bg-ods-card border border-ods-border rounded-md flex items-center justify-center z-10'
            }
          >
            <div className="loading-content flex flex-col items-center gap-3">
              <Loader className="animate-spin text-ods-accent" size={32} />
        ...
        {showPoster && poster && !hasError && (
          <div
            className={
              useNativeAspectRatio ? 'relative cursor-pointer group z-20' : 'absolute inset-0 cursor-pointer group z-20'
            }
