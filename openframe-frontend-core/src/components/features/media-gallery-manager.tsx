...
import { Button, Card, Modal, ModalHeader, ModalTitle } from '../ui';
  ...
  const renderMediaItem = useCallback(
    (mediaItem: MediaItem, index: number) => {
      ...
      return (
        <Card
          key={index}
          className="relative group border-ods-border hover:border-[var(--ods-open-yellow-base)]/30 transition-colors"
          draggable
        ...
        >
          ...
          {/* Media Content */}
          <div className="aspect-video relative bg-[var(--color-text-on-accent)] rounded-lg overflow-hidden">
          ...
          <div className="p-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-ods-text-primary capitalize">{mediaItem.media_type}</span>
            </div>
            {mediaItem.title && <p className="text-sm text-ods-text-secondary truncate">{mediaItem.title}</p>}
          </div>
        </Card>
      );
    },
      ...
      {media.length === 0 ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-['DM_Sans'] font-semibold text-ods-text-primary">Media Gallery ({media.length})</h3>
