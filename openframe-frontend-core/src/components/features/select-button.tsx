    ...
    return (
      ...
      >
        {icon && (
          <span
            className={cn(
              'shrink-0 flex items-center justify-center size-4 md:size-6',
        ...
        <span className="flex flex-1 flex-col justify-center min-w-0 overflow-hidden">
          <span className="md:text-[18px] text-[14px] font-medium text-ods-text-primary truncate">{title}</span>
          {description && (
            <span
              className={cn(
        ...
        {selected && <CheckCircleIcon className="shrink-0 size-4 md:size-6 text-ods-accent" />}
      </button>
    );
  },
);
