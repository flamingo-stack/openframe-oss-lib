...
export const ParallaxImageShowcase: React.FC<ParallaxImageShowcaseProps> = ({
  ...
  // biome-ignore lint/style/useNamingConvention: trailing underscore avoids shadowing framer-motion's scrollY
  ...
  // Grid Layout: Gentler transforms - only vertical movement
  const scrollYGrid = useTransform(scrollY, [0, 1000], [0, -3 * INTENSITY]);
  ...
  const yGrid = useTransform([scrollYGrid, mouseTransformYGrid], ([s, m]) => (s as number) + (m as number));
  ...
  if (layout === 'grid') {
    return (
      <div ref={componentRef} className={`relative w-full h-full flex items-center ${className}`}>
  ...
  if (layout === 'boxed') {
    return (
      <div ref={componentRef} className={`relative w-full h-full overflow-hidden ${className}`}>
        {/* Row 1: Logo Component */}
        {logoElement && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 scale-50">{logoElement}</div>}

        {/* Row 2: Two images with overlaps and margin from row 1 */}
        <div className="absolute top-32 inset-x-0 h-80">
          {/* Left half: Image overlaps to the left */}
          {leftImage && (
            <motion.div className="absolute left-20 lg:w-1/2 w-full h-full z-10" style={{ x: x, y: y, rotate: rotate }}>
              <img
                src={leftImage.src}
                alt={leftImage.alt}
  ...
  return (
    <div ref={componentRef} className={`relative w-full h-full ${className}`}>
      ...
      {centerImage && (
        ...
        >
          <img src={centerImage.src} alt={centerImage.alt} className="w-full h-full object-contain" />
        </motion.div>
      )}
