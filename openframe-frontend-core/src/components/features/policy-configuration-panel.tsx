  ...
  return (
    <div className="bg-ods-bg border-b border-ods-border flex gap-4 items-center px-4 py-3">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <p className="text-[16px] font-medium text-ods-text-primary truncate">{policy.name}</p>
        <p className="text-[12px] text-ods-text-secondary break-all font-mono">{policy.commandPattern}</p>
      </div>
...
}> = ({ category, editMode, onCategoryToggle, onGlobalPermissionChange, onPolicyPermissionChange }) => {
  ...
  return (
    <>
      ...
      >
        {/* Category Icon */}
        <div className="bg-ods-bg border border-ods-border rounded-md flex items-center justify-center w-8 h-8">
          <div className="text-ods-text-secondary">{category.icon}</div>
        </div>

        {/* Category Info */}
        <div className="flex-1 flex flex-col">
          <p className="md:!text-sm font-medium text-ods-text-primary">{category.name}</p>
          <p className="text-[14px] text-ods-text-secondary">{category.configurationsCount} Configurations</p>
        </div>
        ...
        {editMode ? (
          ...
          <div className="px-3 py-2 w-[256px]" onClick={e => e.stopPropagation()}>
      ...
      <div
        ...
        className={cn('overflow-hidden', category.isExpanded ? 'opacity-100' : 'opacity-0')}
      >
...
export const PolicyConfigurationPanel: React.FC<PolicyConfigurationPanelProps> = ({
  ...
  return (
    <div className={cn('bg-ods-card border border-ods-border rounded-md overflow-hidden', className)}>
