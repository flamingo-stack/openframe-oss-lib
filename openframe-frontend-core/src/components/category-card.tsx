...
export function CategoryCard({ name, description, categoryCount, productCount, icons = [] }: CategoryCardProps) {
  return (
    <article className="bg-ods-card border border-ods-border rounded-[12px] p-8 flex flex-col min-w-0 box-border">
      <div className="flex gap-6 mb-8 justify-center items-center">
        {(icons.length > 0 ? icons : Array(10).fill(null)).map((icon, i) => (
          <div key={i} className="w-10 h-10 bg-ods-skeleton rounded flex items-center justify-center">
            {icon}
          </div>
        ))}
      </div>
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex flex-row items-start">
          <p className="text-h6 text-ods-text-primary text-left flex-1">{description}</p>
          <button
            className="w-12 h-12 flex items-center justify-center border border-ods-border rounded-[6px] bg-transparent hover:bg-ods-accent transition-colors ml-4"
