...
export interface TicketOpenFormProps {
  ...
  onSubmit: (input: { subject: string; content: string; attachments: ChatAttachment[] }) => Promise<boolean>;
  isSubmitting: boolean;
  supportSystemDown: boolean;
}

export function TicketOpenForm({ onSubmit, isSubmitting, supportSystemDown }: TicketOpenFormProps) {
    ...
    <Card className="p-6">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 md:flex-row">
        <div className="min-w-0 flex-1 md:max-w-md">
          ...
          {supportSystemDown && (
            <p className="mt-4 text-ods-error text-h6">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          <div>
            <label htmlFor="ticket-subject" className="mb-1 block text-ods-text-primary text-h6">
              Ticket Subject
            </label>
            <Input
              id="ticket-subject"
          ...
          <div>
            <label htmlFor="ticket-content" className="mb-1 block text-ods-text-primary text-h6">
              Your Message
            </label>
            <Textarea
              id="ticket-content"
            ...
            />
            {showCounter && (
              <p className={`mt-1 text-right text-h6 ${overCap ? 'text-ods-error' : 'text-ods-text-secondary'}`}>
