...
describe('ClaudeEmbed', () => {
  ...
  it('SANDBOXES the artifact, and never grants it top-navigation', () => {
    ...
    if (!iframe) throw new Error('expected the artifact to be framed');
    ...
    // never ours.
    expect(sandbox.split(' ').sort()).toEqual(['allow-forms', 'allow-popups', 'allow-same-origin', 'allow-scripts']);
  ...
  it('a NON-embeddable artifact keeps the action and shows the shared empty state', () => {
    const { container } = render(<ClaudeEmbed url={CODE_ARTIFACT} />);
    expect(container.querySelector('iframe')).toBeNull();
    screen.getByText(/Open this one in Claude/);
  ...
  it('occupies the SAME height as a figma embed, framed or not', () => {
    ...
    const figmaBox = render(
      <FigmaEmbed url="https://www.figma.com/design/K1/x" height="70vh" />,
    ).container.querySelector('[style*="70vh"]');
    const framed = render(<ClaudeEmbed url={PUBLISHED} height="70vh" />).container.querySelector('[style*="70vh"]');
    const empty = render(<ClaudeEmbed url={CODE_ARTIFACT} height="70vh" />).container.querySelector('[style*="70vh"]');
  ...
  it("uses the author's name when there is one, and the kind otherwise", () => {
    expect(render(<ClaudeEmbed url={PUBLISHED} title="Suppression brief" />).container.textContent).toContain(
      'Suppression brief',
    );
