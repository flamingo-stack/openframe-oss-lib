...
import { type OnboardingStepConfig, useOnboardingState } from '../../../hooks/ui/use-onboarding-state';
  ...
  return (
    <div className={cn('w-full space-y-4', className)}>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-0">
        <h2 className="text-h2 tracking-[-0.48px] text-ods-text-primary">Get Started</h2>

        {isLoadingCompletion ? (
          <div className="h-[48px] w-full md:w-[160px] bg-ods-border rounded-[6px] animate-pulse" />
        ) : isAllComplete ? (
          <Button variant="primary" onClick={handleDismiss} className="w-full md:w-auto">
            Close Onboarding
          </Button>
        ) : (
          <Button variant="device-action" onClick={handleDismiss} className="w-full md:w-auto">
            Skip Onboarding
          </Button>
        )}
      </div>
