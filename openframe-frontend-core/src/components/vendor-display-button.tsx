      ...
      >
        {logoUrl ? (
          ...
          <div className="w-5 h-5 rounded bg-[#2A2A2A] flex items-center justify-center flex-shrink-0">
            <span className="text-[#666666] text-[10px] font-medium">{vendor.title.charAt(0).toUpperCase()}</span>
          </div>
        )}
        <span className="text-sm font-medium text-white">{vendor.title}</span>
      </button>
    );
  }
    ...
    >
      {getVendorLogo(vendor) ? (
        ...
        <div className="w-8 h-8 bg-ods-border border border-ods-border rounded-lg flex items-center justify-center flex-shrink-0">
          <span className="text-ods-text-primary text-[12px] font-medium">{vendor.title.charAt(0)}</span>
        </div>
      )}
      <span className="text-h4 text-ods-text-primary truncate min-w-0">{vendor.title}</span>
    </button>
  );
}
