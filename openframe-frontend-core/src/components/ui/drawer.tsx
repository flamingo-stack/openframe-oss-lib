  ...
  'fixed z-[9998] flex data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-300',
  {
    variants: {
      side: {
        ...
        left: 'inset-y-0 left-0 items-center pl-4 py-4 data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left',
        top: 'inset-x-0 top-0 justify-center pt-4 px-4 data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top',
        bottom:
...
const drawerPanelVariants = cva('flex flex-col gap-4 rounded-md border border-ods-border bg-ods-card p-4', {
  variants: {
    side: {
      right: 'h-full',
      left: 'h-full',
...
const DrawerContent = React.forwardRef<React.ComponentRef<typeof DialogPrimitive.Content>, DrawerContentProps>(
  ({ side = 'right', className, children, ...props }, ref) => (
    <DrawerPortal>
      <DrawerOverlay />
      <DialogPrimitive.Content ref={ref} className={cn(drawerVariants({ side }))} {...props}>
        <div className={cn(drawerPanelVariants({ side }), className)}>{children}</div>
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn('min-w-0 break-words font-sans text-sm font-medium leading-5 text-ods-text-secondary', className)}
    {...props}
  />
));
