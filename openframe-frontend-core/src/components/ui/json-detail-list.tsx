...
export function JsonDetailList({ data, depth = 0 }: { data: unknown; depth?: number }) {
  const obj =
    typeof data === 'string'
      ? (() => {
          try {
  ...
  return (
    <dl className={`space-y-[var(--spacing-system-xsf)]${depth > 0 ? ' pl-[var(--spacing-system-sf)] border-l border-ods-border' : ''}`}>
