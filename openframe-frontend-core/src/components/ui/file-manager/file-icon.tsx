...
import { File, FileArchive, FileAudio, FileCode, FileText, FileVideo, Folder, Image } from 'lucide-react';
...
const getFileIcon = (extension?: string) => {
  ...
  // Code files
  if (['js', 'ts', 'jsx', 'tsx', 'py', 'java', 'c', 'cpp', 'cs', 'html', 'css', 'json', 'xml'].includes(ext))
    return FileCode;
...
export function FileIcon({ type, extension, className, size = 'md' }: FileIconProps) {
  ...
  return (
    <Icon className={cn(sizeMap[size], type === 'folder' ? 'text-ods-accent' : 'text-ods-text-secondary', className)} />
  );
}
