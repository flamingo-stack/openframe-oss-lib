      ...
      <div className="flex gap-4 items-center w-full">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="flex gap-1 items-center">
            ...
            <span className="text-h4 text-ods-text-primary truncate">{device.name}</span>
          </div>
          {device.organization && (
        ...
        {actions.customActions?.map(
          (action, index) =>
            action.visible !== false && (
              ...
              >
                <span className="text-h3 text-ods-text-primary text-nowrap tracking-[-0.36px]">{action.label}</span>
              </div>
            ),
        )}
      </div>
