...
`fixedHeight` holds `collapsedHeight` (px or any CSS length) even for short content, measures overflow instead of taking `hiddenCount`, and always reserves the toggle row, so the block's height changes only when the reader expands it. It never scrolls inside. `labels` overrides the toggle wording and `toggleClassName` its spacing. `CommandBox visibleLines` is built on it.
