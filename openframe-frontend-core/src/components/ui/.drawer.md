...
| `size` | `"default" \| "medium" \| "wide"` | `"default"` | Panel size preset; `medium` is a 32rem reading panel (capped to the viewport minus gutters), `wide` is 90% of the viewport. Use it instead of a width class. Not combinable with `resizable` (the handle owns the size; a type error) |
