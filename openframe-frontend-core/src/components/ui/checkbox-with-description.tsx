...
const CheckboxWithDescription = React.forwardRef<HTMLDivElement, CheckboxWithDescriptionProps>(
  ({ id, checked, onCheckedChange, title, description, disabled, className }, ref) => (
    ...
    >
      ...
      >
        <CheckboxPrimitive.Indicator className={cn('flex items-center justify-center text-ods-bg')}>
      ...
      <div className="flex flex-col gap-1">
        <Label
          ...
          variant="small"
          spacing="tight"
          className={cn('leading-none cursor-pointer', disabled && 'cursor-not-allowed')}
        >
          {title}
        </Label>
        <span className="text-sm text-ods-text-secondary leading-relaxed">{description}</span>
      </div>
    </div>
  ),
);
