    ...
    >
      <CheckboxPrimitive.Root
        ...
        className={cn(
          ...
          checked ? 'bg-ods-accent border-ods-accent' : 'bg-ods-card border-ods-text-secondary',
        )}
      ...
      <span className="flex-1 text-sm font-medium text-ods-text-primary font-['DM_Sans']">{option.label}</span>
...
export function MobileFilterSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="bottom"
        className={cn('bg-ods-bg border-t border-ods-border p-6 max-h-[85vh] overflow-y-auto', className)}
      >
        <SheetHeader className="mb-4">
          <SheetTitle className="text-h2 text-ods-text-primary tracking-[-0.48px]">{title}</SheetTitle>
        </SheetHeader>
          ...
          {filterGroups.map(group => {
            ...
            return (
              <div key={group.id} className="flex flex-col gap-1">
                <span className="text-h5 text-ods-text-secondary tracking-[-0.24px]">{group.title}</span>
                <div className="rounded-[6px] border border-ods-border overflow-hidden">
                  {group.options.map((option, index) => {
                    const isChecked = groupSelection.includes(option.id);

                    return (
                      <div key={option.id} className={cn(index === group.options.length - 1 && 'border-b-0')}>
                        <FilterCheckboxItem
                          option={option}
                          checked={isChecked}
  ...
  return (
    ...
    >
      <ModalHeader className="px-6 py-6 border-b-0">
        <ModalTitle className="text-h2 text-ods-text-primary tracking-[-0.48px]">{title}</ModalTitle>
      </ModalHeader>

      <ModalContent className="px-6 flex flex-col gap-4 overflow-y-auto min-h-0">
        {sortConfig && sortConfig.columns.length > 0 && (
          <div className="flex flex-col gap-1">
            <span className="text-h5 text-ods-text-secondary tracking-[-0.24px]">{sortConfig.title || 'Sort By'}</span>
            <div className="rounded-[6px] border border-ods-border overflow-hidden">
              {sortConfig.columns.map((column, index) => (
                <SortColumnItem
                  key={column.key}
