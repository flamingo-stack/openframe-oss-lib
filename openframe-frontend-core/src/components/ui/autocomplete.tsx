...
function AutocompleteInner<T = string>(props: AutocompleteProps<T>, ref: React.ForwardedRef<HTMLDivElement>) {
  const {
    options,
    disabled = false,
    startAdornment,
  ...
  const valueArray: T[] = multiple ? (props.value as T[]) : props.value != null ? [props.value as T] : [];

  // biome-ignore lint/correctness/useHookAtTopLevel: hooks are at component top level
      ...
      >
        <ScrollAreaPrimitive.Root className="overflow-hidden">
          <ScrollAreaPrimitive.Viewport className="max-h-[240px] w-full">
            <div role="listbox">
              {loading ? (
                <div className="px-3 py-2 text-ods-text-secondary text-[14px]">{loadingText}</div>
