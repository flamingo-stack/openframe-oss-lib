    ...
    () => ({
      ...
      blockquote: ({ children }: any) => (
        <blockquote className="border-l-4 border-ods-accent ml-0 pl-6 my-8 py-4 rounded-r-lg bg-ods-bg-secondary">
          <div className="font-sans text-[1.125em] leading-relaxed text-ods-text-secondary">{children}</div>
        </blockquote>
      ),
      ...
      // Style lists
      ul: ({ children }: any) => (
        <ul className="list-disc list-outside my-3 ml-6 space-y-1 text-ods-text-primary">{children}</ul>
      ),
      ol: ({ children }: any) => (
        <ol className="list-decimal list-outside my-3 ml-6 space-y-1 text-ods-text-primary">{children}</ol>
      ),
      li: ({ children }: any) => <li className="text-[18px] leading-relaxed pl-1">{children}</li>,

      // Style horizontal rules
      hr: () => <hr className="border-0 border-t my-6 border-ods-border" />,
    }),
    [],
  );
