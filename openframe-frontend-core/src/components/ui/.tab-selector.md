...
Inside an `ActionsMenu` header row (`ActionsMenuHeaderContext`) the selector renders FLUSH: no border, radius, padding or gap of its own, segments square - the menu's row border and rounding are the frame (the mobile "…" menu design).
