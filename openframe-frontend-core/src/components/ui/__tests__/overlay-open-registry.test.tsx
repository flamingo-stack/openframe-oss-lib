...
import { OverlayOpenRegistryProvider, useReportOverlayOpen } from '../overlay-open-registry';
...
describe('OverlayOpenRegistry', () => {
  ...
  it('a stable context value keeps overlays from re-rendering on count changes', () => {
    ...
    function Harness() {
      ...
      // Published from an effect, not during render. Assigning to a binding
      // outside the component while rendering is a render side effect, and it
      // would also run twice per render under StrictMode.
      useEffect(() => {
        bump = () => setTick(t => t + 1);
      }, []);
      return (
