...
export function QueryReportTableSkeleton({
  ...
  return (
    <div className={cn('flex flex-col', isCompact ? 'gap-0' : 'gap-2', className)}>
      {/* Skeleton header */}
      <div className={cn('flex items-center gap-4 px-4', isCompact ? 'py-2 border-b border-ods-border' : 'py-3')}>
        {Array.from({ length: columns }).map((_, i) => (
          <div key={`header-${i}`} className="shrink-0" style={{ width: columnWidth }}>
