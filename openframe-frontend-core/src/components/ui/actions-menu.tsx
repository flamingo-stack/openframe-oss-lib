...
const MenuItem: React.FC<MenuItemProps> = ({ item, onItemClick, isNested = false, parentCloseHandler }) => {
  ...
  const itemClasses = `
    ...
    bg-ods-card
    ${item.disabled ? 'text-ods-text-secondary cursor-not-allowed' : 'text-ods-text-primary hover:bg-ods-bg-hover'}
    ${showSubmenu && item.type === 'submenu' ? 'bg-ods-bg-hover' : ''}
    ...
    <div className="relative">
      <div ref={itemRef} className={itemClasses} onClick={handleClick}>
        ...
        {item.showExternalLinkOnHover && item.href && !item.disabled && (
          <span className="opacity-0 group-hover:opacity-100 transition-opacity" onClick={handleExternalClick}>
            <ExternalLink className="w-5 h-5 text-ods-text-secondary hover:text-ods-text-primary transition-colors cursor-pointer" />
          </span>
        )}
        ...
        {item.type === 'checkbox' && (
          <div
            ...
            ${item.checked ? 'bg-ods-open-yellow' : 'border-2 border-ods-border bg-transparent'}
          `}
          >
        ...
        {item.type === 'submenu' && <ChevronRight className="w-6 h-6 text-ods-text-secondary" />}
      </div>
...
const GroupSeparator: React.FC = () => <div className="bg-ods-system-greys-soft-grey h-1 w-full" />;

export const ActionsMenu: React.FC<ActionsMenuProps> = ({ groups, className = '', onItemClick }) => {
  return (
    <div
      className={`relative min-w-[256px] bg-ods-bg border border-ods-border rounded-md shadow-lg ${className}`}
    >
      {groups.map((group, groupIndex) => (
        <React.Fragment key={groupIndex}>
          {group.items.map((item, itemIndex) => (
            <MenuItem key={item.id || `${groupIndex}-${itemIndex}`} item={item} onItemClick={onItemClick} />
          ))}
