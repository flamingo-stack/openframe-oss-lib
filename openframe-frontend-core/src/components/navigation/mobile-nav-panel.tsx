...
export function MobileNavPanel({ isOpen, config }: MobileNavPanelProps) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[9998] bg-black/50" onClick={config.onClose} />
      ...
      >
        ...
        {config.footer && <div className="border-t border-ods-border p-4 flex-shrink-0">{config.footer}</div>}
      </div>
    </>
  );
}
