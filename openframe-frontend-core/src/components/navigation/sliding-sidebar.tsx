...
import { NavigationItem, SlidingSidebarConfig } from '../../types/navigation';
    ...
    if (hasChildren) {
      ...
      return (
        <div key={item.id} className="space-y-1">
          ...
          >
            {item.label}
            {item.badge !== undefined && <span className="ml-auto mr-2">{item.badge}</span>}
          </Button>
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                <div className="space-y-1 pl-4">{item.children!.map(child => renderMenuItem(child, level + 1))}</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">{config.items.map(item => renderMenuItem(item))}</nav>

        {/* Footer */}
        {config.footer && <div className="p-4 border-t border-ods-border">{config.footer}</div>}
      </motion.aside>
    </>
  );
}
