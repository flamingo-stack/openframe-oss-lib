      ...
      >
        ...
        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <span className="text-lg leading-6 text-white font-['DM_Sans'] font-medium">{title}</span>
            {details && (
              <button
                type="button"
              ...
              >
                <ChevronDown
                  className={cn('w-5 h-5 text-white transition-transform duration-200', isExpanded && 'rotate-180')}
                />
              </button>
            )}
          </div>
          {details && isExpanded && (
            <span className="text-sm leading-5 text-white font-['DM_Sans'] font-medium mt-1">{details}</span>
          )}
        </div>
      </div>
    );
