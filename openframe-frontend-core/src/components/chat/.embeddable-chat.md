...
suggested prompts + source chips) from the public `/api/ai-agents/[slug]`
endpoint, which is byte-compatible with the `EmptyStateConfig` wire shape
(`{ greeting, suggestedQueries, enabledRagTableIds }`).
