...
describe('buildDiscussPrompt', () => {
  it('guide/SSE shape — prose only, retrieval rides entityIdFilter', () => {
    expect(buildDiscussPrompt(ROADMAP_REF)).toBe('Tell me more about Deep Google Workspace tenant management');
  });
  ...
  it('missing / blank title falls back to "this item"', () => {
    ...
    expect(buildDiscussPrompt({ type: 'blog_post', id: 'b1', title: '   ' })).toBe('Tell me more about this item');
  });
