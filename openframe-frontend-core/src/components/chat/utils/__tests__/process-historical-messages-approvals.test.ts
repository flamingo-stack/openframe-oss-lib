...
describe('Product Guide approvals — history and realtime agree', () => {
  ...
  it('renders inline on reload despite a tier list that excludes it', () => {
    const { messages, escalatedApprovals } = processHistoricalMessagesWithErrors([guideProposalMessage('m1')], {
  ...
  it('carries the same fields and origin as the live stream', () => {
    ...
    const guideChunk = decodeNatsChunk({
      type: 'GUIDE',
      payload: {
        kind: 'approval_request',
        proposalId: 'prop-1',
    ...
    // A decoder that declines this frame is the failure the test exists to
    // catch — say so rather than applying `undefined` to the reducer.
    if (!guideChunk) throw new Error('decodeNatsChunk did not decode the GUIDE approval_request frame');
    live.apply(guideChunk);
