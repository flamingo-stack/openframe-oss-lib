...
describe('decideNewTab fallback (no targetPlatform)', () => {
  it('keeps same-page hash CTAs in the same tab', () => {
    expect(decideNewTab({ href: '#next-generations', currentSource: 'flamingo' })).toBe(false);
  });

  it('still opens explicit-host URLs in a new tab', () => {
    expect(decideNewTab({ href: 'https://example.com', currentSource: 'flamingo' })).toBe(true);
  });
});
