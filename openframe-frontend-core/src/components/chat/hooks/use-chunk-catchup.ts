    ...
    async (fromSequenceId?: number | null) => {
      ...
      try {
        ...
        if (lastMessageStartSeqId !== null) {
          ...
          chunksToProcess = uniqueAllChunks.filter(
            item => item.chunk.sequenceId !== undefined && item.chunk.sequenceId >= lastMessageStartSeqId!,
          );
        } else if (lastMessageEndSeqId !== null) {
          // Process only after the last complete message
          chunksToProcess = uniqueAllChunks.filter(
            item => item.chunk.sequenceId !== undefined && item.chunk.sequenceId > lastMessageEndSeqId!,
          );
        } else {
          // Process all
