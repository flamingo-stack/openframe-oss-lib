...
/**
 * `segments` is optional on `UnifiedChatMessage` (user bubbles carry plain
 * `content`). Every assertion below is about an assistant bubble that the
 * updater is supposed to have populated, so a missing `segments` IS the
 * failure — surface it by name instead of letting a non-null assertion turn
 * it into "Cannot read properties of undefined" three lines later.
 */
const segmentsOf = (message: UnifiedChatMessage): MessageSegment[] => {
  const { segments } = message;
  if (!segments) throw new Error(`expected message "${message.id}" to have segments`);
  return segments;
};

describe('appendToTrailingAssistant', () => {
  it('appends into the trailing assistant bubble without dropping prior segments', () => {
    const prev = [user('u1', 'hi'), assistant('a1', [{ type: 'text', text: 'done. ' }])];
...
describe('applyToolExecutionToMessages', () => {
  it('replaces a matching EXECUTING segment in place (same bubble intact)', () => {
    const prev = [user('u1', 'run it'), assistant('a1', [{ type: 'text', text: 'running…' }, executing('exec-1')])];
    const next = applyToolExecutionToMessages(prev, executed('exec-1'));
    expect(next).toHaveLength(2);
    const segs = segmentsOf(next[1]);
  ...
  it('updates a tool in an EARLIER bubble, not just the trailing one', () => {
    const prev = [assistant('a1', [executing('exec-1')]), assistant('a2', [{ type: 'text', text: 'meanwhile' }])];
    const next = applyToolExecutionToMessages(prev, executed('exec-1'));
    expect((segmentsOf(next[0])[0] as ToolExecutionSegment).data.type).toBe('EXECUTED_TOOL');
  ...
  it('merges execution state into an approval_batch containing the exec id', () => {
    ...
    const next = applyToolExecutionToMessages(prev, executed('exec-1'));
    const batch = segmentsOf(next[0])[0];
