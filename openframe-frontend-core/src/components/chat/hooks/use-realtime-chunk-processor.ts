...
import type { ChatApprovalStatus, UseRealtimeChunkProcessorOptions, UseRealtimeChunkProcessorReturn } from '../types';
import { parseChunkToAction } from '../utils/chunk-parser';
import { createMessageSegmentAccumulator, MessageSegmentAccumulator } from '../utils/message-segment-accumulator';

/**
 * Hook for processing real-time NATS chunks into message segments
 */
export function useRealtimeChunkProcessor(options: UseRealtimeChunkProcessorOptions): UseRealtimeChunkProcessorReturn {
    ...
    (chunk: unknown) => {
      ...
      switch (action.action) {
        ...
        case 'approval_result': {
          const { requestId, approved, approvalType: _approvalType } = action;
          ...
          if (escalatedData) {
            ...
            const segments = accumulator.updateApprovalStatus(requestId, approved ? 'approved' : 'rejected');
            callbacks.onSegmentsUpdate?.(segments);
          }
          break;
        }
  ...
  const updateApprovalStatus = useCallback((requestId: string, status: ChatApprovalStatus) => {
