 ...
 */
const GuideDisplay = forwardRef<HTMLDivElement, GuideDisplayProps>(({ className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
    ...
    >
      <div className="flex items-center gap-[var(--spacing-system-xs)]">
        ...
        <span className="text-ods-open-yellow text-h5">{GUIDE_LABEL}</span>
      </div>
      <div className="w-full min-w-0 break-words text-ods-text-primary text-h4">{children}</div>
    </div>
  );
});
