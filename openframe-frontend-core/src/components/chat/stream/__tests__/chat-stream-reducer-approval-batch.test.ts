...
import type { ApprovalBatchSegment, ApprovalResolutionHandler } from '../../types';
...
function findBatch(r: ReturnType<typeof createChatStreamReducer>): ApprovalBatchSegment {
  ...
  const batch = trailing.segments?.find((s): s is ApprovalBatchSegment => s.type === 'approval_batch');
...
describe('SSE kernel — wire-native approval batch', () => {
  it('renders ONE approval_batch segment for a toolCalls event', () => {
    ...
    r.apply(batchEvent);
    const trailing = r.state.messages[r.state.messages.length - 1];
    expect(trailing.segments?.filter(s => s.type === 'approval_request').length ?? 0).toBe(0);
    const batch = findBatch(r);
    expect(batch.status).toBe('pending');
    expect(batch.data.approvalRequestId).toBe('batch:prop-1');
    expect(batch.data.toolCalls.map(c => c.toolExecutionRequestId)).toEqual(['prop-1', 'prop-2']);
  });

  it('onApprove flips status optimistically and resolves every row sequentially', async () => {
    // Typed to the handler contract the reducer actually invokes, so
    // `mock.calls` carries `[requestId?: string]` — an untyped `vi.fn()` makes
    // every recorded argument `any` and the per-row id assertion below pins
    // nothing.
    const onApprove = vi.fn<ApprovalResolutionHandler>().mockResolvedValue(undefined);
...
describe('decoder — approval_batch frame mapping', () => {
  it('maps the wire frame to ONE batched approval-request event', () => {
    ...
    const approval = events.find(e => e.type === 'approval-request') as
      { requestId: string; toolCalls?: Array<Record<string, unknown>> } | undefined;
    // Throwing rather than `expect(...).toBeTruthy()` so the rest of the
    // assertions get a narrowed `approval` — and so a decoder that emits
    // nothing fails by name instead of by TypeError on the next line.
    if (!approval) throw new Error('decoder emitted no approval-request event for the batch frame');
