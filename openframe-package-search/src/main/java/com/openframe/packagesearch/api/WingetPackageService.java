...
import com.openframe.packagesearch.api.dto.PackageSearchItem;
import com.openframe.packagesearch.api.dto.PackageSearchResult;
import com.openframe.packagesearch.api.dto.PackageVersion;
import com.openframe.packagesearch.api.PackageNotFoundException;
import com.openframe.packagesearch.api.PackageSourceUnavailableException;
...
public class WingetPackageService implements PackageManagerClient {
    ...
    private final PackageCatalogReader catalogReader;
    private final RestClient restClient;
    private final Cache<String, PackageDetails> detailsCache;

    public WingetPackageService(PackageCatalogReader catalogReader, PackageSearchProperties properties) {
    ...
    public PackageDetails findPackage(String packageId, BrewPackageType packageType) {
        ...
        List<PackageCatalogEntry> found = catalogReader.findByEntryIds(List.of(entryId));
        if (found.isEmpty()) {
    ...
    private List<Scored> scoreCandidates(String query) {
        List<PackageCatalogEntry> candidates = catalogReader.findCandidates(PackageManagerType.WINGET, query);
