 ...
 * {@link ExecutionMapper} in {@code openframe-api-lib}.
 */
@Component
public class GraphQLExecutionMapper {
    ...
    public CountedGenericConnection<GenericEdge<ExecutionResponse>> toConnection(
            CountedGenericQueryResult<ExecutionResponse> result) {
        List<GenericEdge<ExecutionResponse>> edges = result.getItems().stream()
                .map(view -> GenericEdge.<ExecutionResponse>builder()
                        .node(view)
                        .cursor(CursorCodec.encode(view.getId()))
                        .build())
                .toList();

        return CountedGenericConnection.<GenericEdge<ExecutionResponse>>builder()
                .edges(edges)
