...
class NotificationServiceIT extends BaseMongoIntegrationTest {
    ...
    void list_machine_recipient() {
        ...
        readStateService.createForAudience(n.getId(), NotificationCategory.TICKETS, "title", M, Set.of(MACHINE_1));
    ...
    void audience_isolation_by_id() {
        ...
        readStateService.createForAudience(n.getId(), NotificationCategory.TICKETS, "title", U, Set.of(BOB));
    ...
    void audience_isolation_by_type() {
        Notification n = mongoTemplate.save(NotificationFixtures.basic());
        readStateService.createForAudience(n.getId(), NotificationCategory.TICKETS, "title", U, Set.of("same-id"));
    ...
    void read_filter_narrows_page() {
        ...
        readStateService.createForAudience(n1.getId(), NotificationCategory.TICKETS, "title", U, Set.of(ALICE));
        readStateService.createForAudience(n2.getId(), NotificationCategory.TICKETS, "title", U, Set.of(ALICE));
    ...
    void short_search_normalized_to_no_search() {
        ...
        readStateService.createForAudience(welcome.getId(), NotificationCategory.TICKETS, "title", U, Set.of(ALICE));
        readStateService.createForAudience(alert.getId(), NotificationCategory.TICKETS, "title", U, Set.of(ALICE));
    ...
    @Test
    @DisplayName("Given many read_states for the recipient and a search term that matches none of the denormalized titles, when list is called, then the result is an empty page with hasNextPage=false — search filter pushed into the single flat query, no post-fetch")
    void search_no_match_returns_empty_page() {
        int rowsToSeed = 50;
        List<NotificationReadState> rows = new ArrayList<>(rowsToSeed);
