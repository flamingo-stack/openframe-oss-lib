...
class ScheduleScriptRepositoryIT extends BaseMongoIntegrationTest {
    ...
    void resetCollection() {
        ...
        mongoTemplate.remove(new Query(), ScheduleScript.class);
    }

    private ScheduleScript save(String tenantId, String name, ScriptStatus status,
    ...
    private ScheduleScript saveActive(String tenantId, String name) {
    ...
    void findByTenantIdAndId_isolatesByTenant() {
        ScheduleScript a = saveActive(TENANT_A, "A");
    ...
    void findPageForTenant_noCursor_newestFirst() {
        ScheduleScript s1 = saveActive(TENANT_A, "s1");
        ScheduleScript s2 = saveActive(TENANT_A, "s2");
        ScheduleScript s3 = saveActive(TENANT_A, "s3");

        List<ScheduleScript> page = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "_id", Sort.Direction.DESC, null, false, 10);

        assertThat(page).extracting(ScheduleScript::getId)
    ...
    void findPageForTenant_forwardCursor_olderRows() {
        ...
        assertThat(page).extracting(ScheduleScript::getId).containsExactly(s2.getId(), s1.getId());
    }
    ...
    void findPageForTenant_sortsByRepeatAscending() {
        ScheduleScript weekly = saveWithRepeat(TENANT_A, "weekly", 604800L);
        ScheduleScript halfHour = saveWithRepeat(TENANT_A, "halfHour", 1800L);
        ScheduleScript oneShot = saveWithRepeat(TENANT_A, "oneShot", null);
    ...
    void findPageForTenant_sortsByRepeatDescending() {
        ScheduleScript halfHour = saveWithRepeat(TENANT_A, "halfHour", 1800L);
        ScheduleScript weekly = saveWithRepeat(TENANT_A, "weekly", 604800L);
        ScheduleScript daily = saveWithRepeat(TENANT_A, "daily", 86400L);
    ...
    void findPageForTenant_repeatTiesBrokenById() {
        ScheduleScript a = saveWithRepeat(TENANT_A, "a", 604800L);
        ScheduleScript b = saveWithRepeat(TENANT_A, "b", 604800L);
        ScheduleScript c = saveWithRepeat(TENANT_A, "c", 604800L);

        List<ScheduleScript> desc = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "repeat", Sort.Direction.DESC, null, false, 10);
        // All three tie on repeat → _id DESC decides, and repeats identically across calls.
        assertThat(desc).extracting(ScheduleScript::getId)
                .containsExactly(c.getId(), b.getId(), a.getId());

        List<ScheduleScript> again = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "repeat", Sort.Direction.DESC, null, false, 10);
        assertThat(again).extracting(ScheduleScript::getId)
                .containsExactlyElementsOf(desc.stream().map(ScheduleScript::getId).toList());
    }
    ...
    void findPageForTenant_repeatKeysetPagesCleanlyAcrossTies() {
        ...
        ScheduleScript w1 = saveWithRepeat(TENANT_A, "w1", 604800L);
        ScheduleScript w2 = saveWithRepeat(TENANT_A, "w2", 604800L);
        ScheduleScript w3 = saveWithRepeat(TENANT_A, "w3", 604800L);
        ScheduleScript h1 = saveWithRepeat(TENANT_A, "h1", 1800L);
        ScheduleScript h2 = saveWithRepeat(TENANT_A, "h2", 1800L);

        List<String> walked = new ArrayList<>();
        String cursor = null;
        for (int page = 0; page < 5; page++) {
            List<ScheduleScript> chunk = scheduleRepository.findPageForTenant(
    ...
    void findPageForTenant_repeatKeysetCrossesNullBoundaryAscending() {
        ScheduleScript n1 = saveWithRepeat(TENANT_A, "n1", null);
        ScheduleScript n2 = saveWithRepeat(TENANT_A, "n2", null);
        ScheduleScript half = saveWithRepeat(TENANT_A, "half", 1800L);
        ScheduleScript week = saveWithRepeat(TENANT_A, "week", 604800L);
    ...
    void findPageForTenant_malformedCursorFallsBackToFirstPage() {
        ...
        List<ScheduleScript> noSeparator = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "repeat", Sort.Direction.DESC, "garbage", false, 10);
        List<ScheduleScript> badId = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "repeat", Sort.Direction.DESC, "1800|not-an-objectid", false, 10);
        List<ScheduleScript> badValue = scheduleRepository.findPageForTenant(
    ...
    private ScheduleScript saveWithRepeat(String tenantId, String name, Long repeat) {
        ScheduleScript schedule = ScheduleScript.builder()
                .tenantId(tenantId)
                .name(name)
    ...
    void findPageForTenant_sortsByDeviceCountDesc() {
        ScheduleScript few = saveActive(TENANT_A, "few");
        ScheduleScript many = saveActive(TENANT_A, "many");
        ScheduleScript mid = saveActive(TENANT_A, "mid");
    ...
    void findPageForTenant_sortsByDeviceCountAsc_missingAssignmentIsZero() {
        ScheduleScript none = saveActive(TENANT_A, "none");   // no assignment doc at all
        ScheduleScript some = saveActive(TENANT_A, "some");
        assignMachines(TENANT_A, some.getId(), List.of("m1", "m2"));

        List<ScheduleScript> page = scheduleRepository.findPageForTenant(
                TENANT_A, null, null, "deviceCount", Sort.Direction.ASC, null, false, 10);

        assertThat(page).extracting(ScheduleScript::getId).containsExactly(none.getId(), some.getId());
    ...
    void findPageForTenant_deviceCountCursorPagesCleanlyAcrossTies() {
        ScheduleScript a = saveActive(TENANT_A, "a");
        ScheduleScript b = saveActive(TENANT_A, "b");
        ScheduleScript c = saveActive(TENANT_A, "c");
        ScheduleScript d = saveActive(TENANT_A, "d");
    ...
    void findPageForTenant_deviceCountAggregation_isTenantIsolated() {
        ScheduleScript aOnly = saveActive(TENANT_A, "a-only");
        ScheduleScript bOnly = save(TENANT_B, "b-only", ScriptStatus.ACTIVE, "user-1", List.of(OsType.MAC_OS));
        ...
        assertThat(page).extracting(ScheduleScript::getId).containsExactly(aOnly.getId());
    }
    ...
    void findPageForTenant_excludesDeleted() {
        ...
        assertThat(page).extracting(ScheduleScript::getName).containsExactly("active");
    }
    ...
    void findPageForTenant_isolatesByTenant() {
        ...
        assertThat(page).extracting(ScheduleScript::getName).containsExactly("a1");
    }
