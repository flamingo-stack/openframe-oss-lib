...
class CustomNotificationSettingsRepositoryImplTest {
    ...
    void save_upserts_by_user_id() {
        ...
        repository.saveSettings("alice", false, Set.of(NotificationSettingGroup.MINGO_MESSAGES));
        ...
        assertThat(set).contains("enabled=false").contains("mutedGroups");
        assertThat(update.getValue().getUpdateObject().get("$setOnInsert").toString()).contains("createdAt");
    }

    @Test
    @DisplayName("Given a legacy master-only write (null mutedGroups), when saved, then the stored muted set is NOT touched")
    void null_type_settings_keeps_stored_overrides() {
        ...
        assertThat(update.getValue().getUpdateObject().get("$set").toString()).doesNotContain("mutedGroups");
    }
