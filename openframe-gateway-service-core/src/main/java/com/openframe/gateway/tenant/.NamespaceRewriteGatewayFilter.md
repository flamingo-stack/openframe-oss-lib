<!-- source-hash: f0417489ead0203c3092075bd81c6fcf -->
...
|--------|------|-------------|
| `filter()` | Method | Reads `X-Tenant-Ns`, rewrites the host of `GATEWAY_REQUEST_URL_ATTR` via `GatewayTenantNamespace.applyToUri`, and stores it back; no-op when the header is absent or the host has no namespace label |
