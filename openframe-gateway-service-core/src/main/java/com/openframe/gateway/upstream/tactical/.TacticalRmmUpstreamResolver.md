<!-- source-hash: 62f752ff03fb10176b55e56263a6587f -->
Resolves upstream target URIs for Tactical-RMM tool requests within the OpenFrame gateway, routing traffic to one of three independent backends (REST/Django, NATS WebSocket, or Daphne ASGI) based on request type and path. On a shared multi-tenant pod it rewrites the upstream namespace for the calling tenant per request.
...
| `build()` | Method | Builds the proxy `URI` from the chosen upstream, then applies the per-request tenant namespace via `GatewayTenantNamespace.applyToUri` (no-op without `X-Tenant-Ns`) |
...
On a shared pod the configured upstream hosts carry a `tenant-ns` placeholder (e.g. `tactical-backend.tenant-ns.svc.cluster.local`). `build()` swaps that label for the calling tenant's namespace, read from the trusted `X-Tenant-Ns` header. Single-tenant pods send no such header, so the configured host is used verbatim.
