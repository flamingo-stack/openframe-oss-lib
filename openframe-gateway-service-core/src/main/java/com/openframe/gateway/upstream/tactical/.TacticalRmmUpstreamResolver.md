<!-- source-hash: 55a4ea349587c3805085282d03188e06 -->
...
| `build()` | Method | Builds the proxy `URI` from the chosen upstream, then applies the per-request tenant namespace via `TenantRoutingHeaders.applyToUri` (no-op without `X-Tenant-Ns`) |
