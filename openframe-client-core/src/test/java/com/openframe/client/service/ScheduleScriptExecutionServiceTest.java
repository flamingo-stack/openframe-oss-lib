...
class ScheduleScriptExecutionServiceTest {
    ...
    void dueSchedule_firesThenAdvancesCadence() {
        ...
        ScheduleScript schedule = due(now.minus(Duration.ofSeconds(6300)), 1800L);
        when(scheduleRepository.findByStatusAndNextRunAtLessThanEqual(eq(ScriptStatus.ACTIVE), any()))
    ...
    void oneShot_clearsNextRun() {
        ScheduleScript schedule = due(Instant.now().minusSeconds(1), null);
        when(scheduleRepository.findByStatusAndNextRunAtLessThanEqual(eq(ScriptStatus.ACTIVE), any()))
                .thenReturn(List.of(schedule));
    ...
    void dispatchFailure_stillAdvancesButDoesNotStampLastRun() {
        Instant now = Instant.now();
        ScheduleScript broken = due(now.minusSeconds(1), 1800L);
        broken.setId("broken");
        ScheduleScript ok = due(now.minusSeconds(1), 1800L);
        ok.setId("ok");
        when(scheduleRepository.findByStatusAndNextRunAtLessThanEqual(eq(ScriptStatus.ACTIVE), any()))
    ...
    private static ScheduleScript due(Instant nextRunAt, Long repeat) {
