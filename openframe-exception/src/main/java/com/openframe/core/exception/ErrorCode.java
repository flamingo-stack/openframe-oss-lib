package com.openframe.core.exception;

import lombok.AllArgsConstructor;
import lombok.Getter;

@Getter
@AllArgsConstructor
public enum ErrorCode {

    // Generic HTTP error codes
    NOT_FOUND("NOT_FOUND", 404),
    BAD_REQUEST("BAD_REQUEST", 400),
    VALIDATION_ERROR("VALIDATION_ERROR", 400),
    UNAUTHORIZED("UNAUTHORIZED", 401),
    FORBIDDEN("FORBIDDEN", 403),
    CONFLICT("CONFLICT", 409),
    INTERNAL_ERROR("INTERNAL_ERROR", 500),
    METHOD_NOT_ALLOWED("METHOD_NOT_ALLOWED", 405),
    UNSUPPORTED_MEDIA_TYPE("UNSUPPORTED_MEDIA_TYPE", 415),

    // Domain-specific error codes
    USER_NOT_FOUND("USER_NOT_FOUND", 404),
    DEVICE_NOT_FOUND("DEVICE_NOT_FOUND", 404),
    API_KEY_NOT_FOUND("API_KEY_NOT_FOUND", 404),
    ENCRYPTION_FAILED("ENCRYPTION_FAILED", 500),
    DECRYPTION_FAILED("DECRYPTION_FAILED", 500),
    ENCRYPTION_ERROR("ENCRYPTION_ERROR", 500),
    AGENT_REGISTRATION_SECRET_NOT_FOUND("AGENT_REGISTRATION_SECRET_NOT_FOUND", 404),
    NO_ACTIVE_SECRET_FOUND("NO_ACTIVE_SECRET_FOUND", 404),
    INITIAL_KEY_EMPTY("INITIAL_KEY_EMPTY", 401),
    INITIAL_KEY_INVALID("INITIAL_KEY_INVALID", 401),
    CLIENT_SECRET_EMPTY("CLIENT_SECRET_EMPTY", 401),
    CLIENT_SECRET_INVALID("CLIENT_SECRET_INVALID", 401),
    OPERATION_NOT_ALLOWED("OPERATION_NOT_ALLOWED", 403),
    OWNER_DELETE_NOT_ALLOWED("OWNER_DELETE_NOT_ALLOWED", 409),

    // External API specific
    LOG_NOT_FOUND("LOG_NOT_FOUND", 404),
    ORGANIZATION_NOT_FOUND("ORGANIZATION_NOT_FOUND", 404),
    CUSTOMER_NOT_FOUND("CUSTOMER_NOT_FOUND", 404),
    ORGANIZATION_HAS_MACHINES("ORGANIZATION_HAS_MACHINES", 409),
    KNOWLEDGE_BASE_ITEM_NOT_FOUND("KNOWLEDGE_BASE_ITEM_NOT_FOUND", 404),
    KNOWLEDGE_BASE_ATTACHMENT_NOT_FOUND("KNOWLEDGE_BASE_ATTACHMENT_NOT_FOUND", 404),

    // Tag specific
    TAG_NOT_FOUND("TAG_NOT_FOUND", 404),
    TAG_ALREADY_EXISTS("TAG_ALREADY_EXISTS", 409),

    // Time tracking specific
    TIME_ENTRY_NOT_FOUND("TIME_ENTRY_NOT_FOUND", 404),
    TIME_ENTRY_ACTIVE_EXISTS("TIME_ENTRY_ACTIVE_EXISTS", 409),
    TIME_ENTRY_NO_ACTIVE("TIME_ENTRY_NO_ACTIVE", 409),
    TIME_ENTRY_RUNNING_NOT_EDITABLE("TIME_ENTRY_RUNNING_NOT_EDITABLE", 409),
    TIME_ENTRY_TICKET_ARCHIVED("TIME_ENTRY_TICKET_ARCHIVED", 409),
    TIME_ENTRY_REQUIRES_CONTENT("TIME_ENTRY_REQUIRES_CONTENT", 400),

    // Ticket specific
    TICKET_NOT_FOUND("TICKET_NOT_FOUND", 404),
    TICKET_NOTE_NOT_FOUND("TICKET_NOTE_NOT_FOUND", 404),
    TICKET_INVALID_TRANSITION("TICKET_INVALID_TRANSITION", 409),
    TICKET_AI_DISABLED("TICKET_AI_DISABLED", 409),
    TICKET_STATUS_LOCKED_BY_APPROVAL("TICKET_STATUS_LOCKED_BY_APPROVAL", 409),
    TICKET_STATUS_NOT_FOUND("TICKET_STATUS_NOT_FOUND", 404),
    TICKET_STATUS_IN_USE("TICKET_STATUS_IN_USE", 409),
    TICKET_STATUS_DUPLICATE_NAME("TICKET_STATUS_DUPLICATE_NAME", 409),
    TICKET_STATUS_SYSTEM_PROTECTED("TICKET_STATUS_SYSTEM_PROTECTED", 403),
    TICKET_STATUS_LIMIT("TICKET_STATUS_LIMIT", 409),
    TICKET_STATUS_INVALID_REORDER("TICKET_STATUS_INVALID_REORDER", 400),
    TICKET_STATUS_INVALID_REPLACEMENT("TICKET_STATUS_INVALID_REPLACEMENT", 400),

    // Authorization specific
    USER_ACTIVE_IN_ANOTHER_TENANT("USER_IS_ACTIVE_IN_ANOTHER_TENANT", 409),
    OWNER_CANNOT_SWITCH_TENANT("OWNER_CANNOT_SWITCH_TENANT", 409),

    // Infrastructure error codes
    TYPE_MISMATCH("TYPE_MISMATCH", 400),
    PINOT_QUERY_ERROR("PINOT_QUERY_ERROR", 503),
    LOKI_QUERY_ERROR("LOKI_QUERY_ERROR", 503),
    DATABASE_ERROR("DATABASE_ERROR", 503);

    private final String code;
    private final int httpStatus;
}
